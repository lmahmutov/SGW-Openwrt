  ...
  switch (p_task->TaskState) {
    case STATE_RUNNING:
      if (GetCurrentThreadId() == p_task->ThreadID) {
        ...
        sli_nvic_trigger_interrupt(0, NULL);
      } else {
...
void OSStartHighRdy(void)
{
  ...
  WaitForSingleObject(OSTimerThread, INFINITE);                 // Wait until OSTick Thread has terminated.
