...
void halHostSerialPowerdown(void)
{
  ...
  // Disable the interrupt on CS so that it won't be triggered by the pin
  // configuration done by SPIDRV_DeInit() on wakeup.
  GPIO_IntDisable(1 << SPI_NCP_CS_PIN);
}
  ...
  switch (getHostByte(SPIP_PREFIX_INDEX)) {
    ...
    case 0xFE: //The Command is an EZSP Frame
      if (getHostByte(SPIP_LENGTH_INDEX) > MAX_PAYLOAD_FRAME_LENGTH) {
        ...
        // the rationale.
        spiHandle->initData.port->CMD = USART_CMD_RXBLOCKEN;
  ...
  switch (spipNcpState.state) {
    case spipNcpIdle:
      ...
      if (spiHandle->state == spidrvStateIdle) {
        ...
        spiHandle->initData.port->CMD = USART_CMD_CLEARRX | USART_CMD_CLEARTX;
        ...
        spiHandle->initData.port->CMD = USART_CMD_RXBLOCKDIS;
        break;
