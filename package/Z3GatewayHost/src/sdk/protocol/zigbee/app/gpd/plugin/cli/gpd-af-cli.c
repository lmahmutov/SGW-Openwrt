...
void emberGpdAfCliNodeSetAddr(void)
{
  ...
  if (gpd->addr.appId == EMBER_GPD_APP_ID_SRC_ID) {
    gpd->addr.id.srcId = (uint32_t)emberUnsignedCommandArgument(1);
  } else if (gpd->addr.appId == EMBER_GPD_APP_ID_IEEE_ID) {
    gpd->addr.gpdEndpoint = (uint8_t)emberUnsignedCommandArgument(1);
    // TODO : To set the IEEE address of the node.
    //uint8_t len = emberCopyBigEndianEui64Argument(2,gpd->addr.id.ieee);
  }
}
void emberGpdAfCliNodeSetBiDirParam(void)
{
  ...
  if (gpd->rxAfterTx) {
    gpd->rxOffset = (bool)emberUnsignedCommandArgument(1);
    gpd->minRxWindow = (bool)emberUnsignedCommandArgument(2);
  }
}
